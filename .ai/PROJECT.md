...
- No application source code, configuration, pipeline, or infrastructure files are present; the repository contains only the context template files (`AGENTS.md`, `README.md`, `.gitignore`, and `.ai/`).
- Primary technologies, cloud providers, target users, workload type, environments, and non-functional requirements are **Unknown / not determined from repository**.
